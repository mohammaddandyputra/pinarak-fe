...
export default function Page() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      errorToastify(
        'Terjadi kesalahan. Gagal melakukan login. Silakan coba lagi nanti.'
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleVerification = async () => {
    ...
    if (fullCode.length === 6) {
      try {
        ...
        if (result?.error) {
          errorToastify(
            'Verifikasi gagal. Silakan periksa kembali kode OTP Anda'
          );
        } else {
          successToastify('Verifikasi berhasil');
          router.push('/');
      ...
      if (response.ok) {
        ...
        if (data?.error) {
          if (
            ...
            errorToastify(
              'Terlalu banyak mencoba. Silakan coba beberapa menit lagi.'
            );
          } else {
            errorToastify(
              'Terjadi kesalahan. Gagal melakukan resend kode. Silakan coba lagi nanti.'
            );
          }
        }
      }
  ...
  return (
    <div
      className='flex justify-center items-center min-h-screen bg-gray-50 bg-cover bg-center'
        ...
        <Card className='w-full max-w-md border border-gray-200'>
          <CardHeader className='flex flex-col items-center space-y-4 pt-8 pb-6'>
            ...
            <p className='text-gray-600 text-center'>
              Masukkan kode 6 digit yang telah dikirim ke email Anda. Kode
          ...
          <CardBody className='px-8 py-10'>
            ...
            <p className='mt-6 text-center text-gray-600'>
              ...
              >
                {canResend
                  ? 'Kirim ulang'
                  : `Kirim ulang dalam ${resendTimeout} detik`}
              </button>
            </p>
          </CardBody>
        </Card>
